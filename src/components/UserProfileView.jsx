...
const UserProfileView = ({ profileUser }) => {
  ...
  useEffect(() => {
    if (!currentUser && !loading) {
      ...
      if (userId) {
        console.log("UserProfileView... dispatching getUserById: ", userId);
  ...
  }, [currentUser, dispatch, loading, localUser]);
  ...
  if (!profileUser) {
    return (
      <div className="text-center p-5">
        ...
        <p className="mt-2">Caricamento profilo in corso...</p>
      </div>
    );
  }
  ...
  const getMemberCommunitiesCount = () => {
    ...
    // Ottieni gli ID delle community create dall'utente
    const createdCommunityIds = profileUser.communities
      ? profileUser.communities.map((community) => community.communityId)
      : [];

    // Conta solo le community in cui l'utente partecipa ma che non ha creato
  ...
  if (!currentUser && !profileUser && !localUser) {
    return (
